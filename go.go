...
func main(){
	LoadMap("G:/www12/mir2ServerRelease/Maps/0.map")
}
...
func LoadMap(filepath string){
	...
	switch v {
	// case 0:
	// 	data := GetMapV0(fileBytes)

	case 1:
		GetMapV1(fileBytes)

	// case 3:
	// 	data := GetMapV3(fileBytes)

	// case 5:
	// 	data := GetMapV5(fileBytes)
	default:
		log.Printf("decoce:%d", int(v))
	}

	// log.Printf("decoce: %d", int(data))
}
...
func GetMapV1(bytes []byte){
	offset := 21
	w := common.BytesToUint16(bytes[offset:])
	log.Printf("w : %d", int(w))
	offset += 2
	...
	log.Printf("decoce:%d", int(width))
	log.Printf("decoce:%d", int(height))
